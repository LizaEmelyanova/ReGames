...
const SnakeGame = () => {
  ...
  // Функция для интерполяции углов
  function interpolateAngle(a0, a1, t) {
    let delta = a1 - a0;
    // Обеспечиваем кратчайший путь поворота
  ...
  function drawCatmullRomCurve(ctx, points) {
  ...
  return (
    <div className="snake-game">
      <h1 className="score-title">
        Баллы <span className="score-bold">{score}</span>
      </h1>
      <canvas ref={canvasRef} width={boardSize} height={boardSize} className="board" />
      {gameOver && (
        <div className="game-over-message">
          <p>Игра окончена</p>
          <p className="game-over-score-message">Ваши баллы: {score}</p>
